...
**The student gets a voice in it (v0.13 follow-up).** `report the generated arrays are not sorted, so O(n) is unachievable` — the words after the command, or `dojo report <slug> --note "…"` from outside a session — travel into the audit prompt under "WHAT THE STUDENT REPORTED", are recorded as `student_note` in the report JSON, and the auditor must address the claim by name. It may disagree: a report is evidence, not a verdict, and "your generator is right — the statement never promises sorted input" is a real answer (the audit prompt says so outright, so a mistaken report cannot manufacture a finding). Both call sites also print the **automated cross-check findings** — fresh-vs-live oracle disagreements — so "no contract violations" and "the cross-check never ran" cannot be mistaken for each other.

**A report can never cost a session (v0.13 follow-up).** `report` runs behind `dojo.guard`: a curator or transport failure prints one line ("the curation audit unavailable (…) — your session is unaffected") and the loop continues. It is available in both phases, since a grading complaint is most natural right after living with the grading. The rule underneath is that model-written code is executed as *shape variance, never a fatal*: `curator.sanitize_imports` drops imports the isolated namespace cannot satisfy (a live crash — the curator read the prompt's import rule as an instruction to write `from decorators import oracle`), reports what it stripped as an automated finding, and `_exec_proposal` raises `CuratorError` for whatever the code itself does.
