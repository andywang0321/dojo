...
**Your workbench and VSCode:** sessions edit files in `~/.local/share/dojo/workbench/` — outside the repo, so editor/debugger tooling never collides with git. Every workbench file starts with a shebang pointing at dojo's venv (so any editor or linter knows which Python it belongs to), and dojo generates a self-contained VSCode workspace *inside* the workbench folder: `dojo open` opens that folder in VSCode-family editors with the debugger already wired — pick "dojo: debug the active workbench file" and press F5. Double-clicking `workbench.code-workspace` does the same. Install any Python tooling like ipykernel into the venv — git ignores it, and updates stay clean.
