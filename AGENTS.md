...
- **Curating a new problem:** add `function_name` + `visible_tests` to `data/problem_overrides.json`, a signature to `SIGNATURES` in `session/flow.py` (until signatures move into overrides in v0.3), and a generator/oracle pair in `judge/registry.py` (a `@profiler_input` too, unless measurement is meaningless). Then `dojo init` re-seeds (an upsert, so it never deletes rows attempts reference). The judge's strict JSON equality means output contracts must be deterministic: pin canonical sorted order where prompts say "any order", "leftmost peak" for multi-peak problems, rounded floats for correlation, and nested lists `[value, left, right]` for trees (`None` = missing child / empty tree). `tests/test_registry.py` pins the contract — update it when the curated set changes. Class-based, multi-function, and predicate-checked problems (`min_stack`, `encode_and_decode_strings`, `generate_sample_to_target_sum`) are deliberately uncurated until the judge protocol grows.
